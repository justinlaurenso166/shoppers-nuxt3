            ...
            <div class="w-[60%] mx-auto py-20">
                <div class="flex gap-10" v-if="!loading">
                    ...
                    <div class="w-1/2">
                        ...
                        <div class="flex mt-4">
                            <button class="flex items-center justify-center text-3xl px-3 py-1 border-primary border-[1.5px] font-light rounded-l-md hover:bg-secondary transition-all duration-150 hover:shadow-xl hover:text-white" @click="changeQty('min')"> - </button>
                            <input type="number" v-model="buy_product.qty" class="w-14 text-center font-light text-md border-[#a8a2f3] border-l-0 border-r-0 border-[0.1px] focus:outline-none focus:border-primary">
                            <button class="flex items-center justify-center text-2xl px-3 py-1 border-primary border-[1.5px] font-light rounded-r-md hover:bg-secondary transition-all duration-150 hover:shadow-xl hover:text-white" @click="changeQty('plus')"> + </button>
                        </div>
                        <div class="mt-10">
                            <button class="w-[30%] uppercase tracking-wider text-white bg-primary hover:bg-secondary transition-all duration-150 hover:shadow-2xl text-sm font-light py-3 rounded-sm" @click="addToCart()">Add To Cart</button>
                        </div>
                    </div>
                </div>
                <div v-else>
